...
python3 scripts/check_figure_text.py <交付包目录>                # 图 ↔ 文书对账 T1–T3（读 figures/*.manifest.json）
...
- `check_figure_labels.py`：附图标记对照表门禁 N1–N4。hard-rules §5 早就写了"附图说明节末必备
  ...
  **为什么自己写而不复用现成实现**（本轮真实检索并逐个打开仓库看过）：
...
- `check_figure_text.py`：图与文书的逐字对账 T1–T3。它不读 PNG 像素、也不让人手抄一份登记表，而是读 `patent_figure.py` 出图时自己写下的 `<图名>.manifest.json`（图上画的 标记号→部件 与每一段框内文字）——手抄件互比只能证明两份抄得像，证明不了图上真写着这个。T1 图上的部件名必须逐字见于文书（图上出现文书没有的部件即判红）；T2 框内文字里的量值（含 ≤/≥/± 与单位）必须逐字见于文书，比较前**只**归一「符号与数字之间的空白」，多删一个空格都不许（部件名中间塞空格想蒙混，正是这条归一范围断言钉住的）；T3 图上的 标记号→部件 必须与那张「标记｜名称｜所在图号」对照表一致——F3 管跨图同号同件，图↔表这一头此前没人管：图写 12=底座、表写 12=支架，两道门各自都绿。三态另说：包里没有 figures 目录只报未判；有 PNG 却没有清单 ⇒ rc=2 说「图不是本库出的，无从查证」，既不折成合规也不折成违规——且这一条与「有没有别的图带着清单」无关：12 张图里混 1 张手画 PNG，那张图上写着什么同样没人核过，包级对账就不完整（判到多少照实报多少，但退码按不完整收）。旧写法把「看孤儿」关在「一张清单都没有」那个分支里，于是这种最像真相的形态读出来是「实判 3 条／违规 0」；它还与配对缺陷互相遮着——清单名 `图1.manifest.json` 是双后缀，用 `os.path.splitext` 取到的是 `图1.manifest`，永远配不上 `图1.png`，合规包会被读成缺清单，而那条判据在有清单时根本不看孤儿。现在配对靠去掉固定的 `.manifest.json` 后缀，两处各有一条常驻断言与一条注入。退码优先级：真判出的违规先报（rc=1，另打一句「另有：不完整在哪」），没判出违规才按 rc=2 收场——把违规降级成环境档等于抹掉一次真开火。md 与 docx 两通道同判据（与 E/G/K/N/V 一致）：常驻用例用 python-docx 造一份带真 w:tbl 的 Word 说明书并删光 md，Word-only 包里 T1–T3 照样实判三条且全绿，把对照表里的部件名改一格 T3 必须开火，读不动的 docx 走 rc=2 而不是崩成"发现违规"——上一轮给四把表门禁接通道时 T 还没出生，这条通道是本轮补上的，端到端脚本里也有一份同样的 Word-only 场景（`.codebuddy/attest/r20_e2e.py` S9）。
