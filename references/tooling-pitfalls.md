...
6. **质量门脚本自身缺陷**：自动判定脚本的正则/传参错误会误报 FAIL（实测缺陷清单里的第 3 项取错列、第 6 项 pytest 多目标传参；这里刻意不写"字母加序号"那种旧编号——那个形状现在会被契约检查读成权要门禁的判据编号，把历史叙述当成虚指的判据）→ 门脚本也要被验证，修复留痕。同类实例见 §2「字节阈值误判合法稀疏框图」——误报不一定是写错正则，也可能是判据选错了度量对象。
