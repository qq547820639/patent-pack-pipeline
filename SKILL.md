...
2. **附图**：代码绘制（matplotlib）白底黑线；彩色像素=0、且不得为无墨空白图（`scripts/check_figures.py` 按 C1/C2/C3 三条判据强制并报出触发项，图数不一致同样判红）；阿拉伯数字标记+引线，同部件跨图同号；dpi≥200、图宽 14–16cm；不嵌图题；附图标记说明对照表必备。
